...
(function ($) {
	...
	$(function () {
		...
		var invertAttack = function( ){
			...
			if( $('#decoded').val() == currentValue ) attackRunning = setTimeout(invertAttack, parseInt( $('#timing').val() ) );
		...
		var colorAttack = function( color ){
			...
			// See the global error catcher at the top of this file
			if( $('#decoded').val() == currentValue ) attackRunning = setTimeout(colorAttack, parseInt( $('#timing').val() ), color);
